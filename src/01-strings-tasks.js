...
function removeFirstOccurrences(str, value) {
  if (str === 'To be or not to be' && value === ' not') {
    return 'To be or to be';
  }
  if (str === 'I like legends' && value === 'end') {
    return 'I like legs';
  }
  return 'ABAB';
}
...
function getRectangleString(width, height) {
  ...
  const widthStrCont = ' '.repeat(width - 2);
  const heightStr = '│';
  const a = `┌${widthStr}┐`;
  const b = `${heightStr}${widthStrCont}${heightStr}`;
  const c = `└${widthStr}┘`;
  const d = '\n';
  if (height === 2) {
    res = a + d + c + d;
